...
## Goals
- Mirror the ergonomics and reliability of the TypeScript `@request-suite/request-api-client` in a PHP-first package that WooCommerce (and other PHP runtimes) can consume without a Node bridge.
...
- Operation coverage: each facade method will be associated with a canonical
  ...
  operations against `@request-suite/request-client-contracts/specs/openapi/request-network-openapi.json`.
